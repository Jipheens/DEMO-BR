...
import { ManageLeaveBandsComponent } from '../dialogs/manage-leave-bands/manage-leave-bands.component';
...
@Component({
  selector: 'app-all-leave-bands',
  templateUrl: './all-leave-bands.component.html',
  styleUrls: ['./all-leave-bands.component.sass']
})
export class AllLeaveBandsComponent implements OnInit {
...
getSelectedStatus(status: any) {
  ...
  mngRecord(action: any, record?: any) {
    ...
    const dialogRef = this.dialog.open(ManageLeaveBandsComponent, dialogConfig);
...
let params = {id: id}
    Swal.fire({
      title: "Are you sure?",
      text: "ANNUAL LEAVE BAND RECORD WILL BE DELETED PERMANENTLY!!",
      icon: "warning",
            ...
            next: (res) => {
              if (res.statusCode == 200) {
                ...
                this.notificationAPI.alertSuccess(
                  "LEAVE BAND RECORD DELETED SUCCESSFULLY"
                );
              } else {
                this.getData();
                this.loading = false;
