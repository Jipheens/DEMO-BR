...
export class RecursiveAllowanceService {

  headers = new HttpHeaders().set('Content-Type', 'application/json');
  baseURL = `${environment.baseUrlHR}/api/v1/payroll/recursive-allowance`;
  ...
  createMultile(data: any): Observable<any> {
  ...
  verify(body: any): Observable<any> {
    ...
    return this.http.put(API_URL, body,{headers: this.headers, withCredentials: false }).pipe(map(res => {
      return res || {}
    }),
      catchError(this.errorMgmt)
    );
  ...
  validateBulkAllowances(processedRows: any): Observable<any> {
