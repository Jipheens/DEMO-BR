...
import { ManageBranchesComponent } from "../../../modules/configurations/branches/dialogs/manage-branches/manage-branches.component";
...
@Component({
  ...
  styleUrls: ["./employees-lookup.component.sass"],
})
export class EmployeesLookupComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.data.departmentCode) {
      this.departmentCode = this.data.departmentCode
    }

    if (this.data.action === "getDsrEmployees") {
      this.getData();
    }else{
      this.getData();
    }
  ...
  getDataDsrEmployees(page?: number, size?: number) {
    this.isLoading = true;
    this.dataSource = new MatTableDataSource([]);
    this.dataSource.paginator = this.paginator;
    this.dataSource.sort = this.sort;
    ...
    this.employeeAPI
      ...
      .subscribe({
        next: (res) => {
          ...
          if (res.statusCode === 302) {
            // this.data = res;
            let dt=res.entity.filter(item => item.enrollmentStatus === 'Direct Sales Representative');
            this.isLoading = false;
