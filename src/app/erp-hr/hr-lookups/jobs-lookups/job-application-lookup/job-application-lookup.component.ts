...
@Component({
  ...
  styleUrls: ['./job-application-lookup.component.sass']
})
export class JobApplicationLookupComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      if (this.data.action === "filteredApplications") {
        this.getJobDataByRefNoAndStatus("SHORTLISTED",this.data.jobRefNumber)
      }else{
        this.getData();
      }
    }
    ...
    getJobDataByRefNoAndStatus(status,jobRefNumber) {
      this.isLoading = true;
      this.dataSource = new MatTableDataSource([]);
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
